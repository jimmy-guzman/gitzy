...
const validateConfig = (
  ...
  if (typeof userConfig === 'object') {
    ...
    if (!isValidType('headerMaxLength', 'number', userConfig)) {
      return errorObject('headerMaxLength must a number')
    }

    if (!isValidType('headerMinLength', 'number', userConfig)) {
      return errorObject('headerMinLength must a number')
    }
